...
for dir1 in $(ls testers); do
    for file in testers/$dir1/*.maps; do
        ...
        if [ "$MODE" == "time" ]; then
                { time ./navigate_p2 "$file"; } 2>&1 | tee -a $MODE.txt
        elif [ "$MODE" == "valgrind" ]; then
            valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --track-fds=yes ./navigate_p2 "$file" 2>&1 | tee -a $MODE.txt
        fi
