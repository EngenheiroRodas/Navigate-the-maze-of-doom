...
for file in testers/*.1maps; do
    ...
    if [ "$MODE" == "time" ]; then
        { time ./navigate_p1 "$file"; } 2>&1 | tee -a "$MODE.txt"
    elif [ "$MODE" == "valgrind" ]; then
        valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --track-fds=yes ./navigate_p1 "$file" 2>&1 | tee -a "$MODE.txt"
    fi
